...
import wj.com.alldemo.BuildConfig;
...
public class AppForegroundStateManager {
    ...
    public static final long APP_CLOSED_VALIDATION_TIME_IN_MS = 1 * DateUtils.SECOND_IN_MILLIS; // 4 Seconds
    ...
    private AppForegroundState mAppForegroundState = AppForegroundState.IN_BACKGROUND;
    ...
    private void determineAppForegroundState() {
        ...
        mAppForegroundState = isInForeground ? AppForegroundState.IN_FOREGROUND : AppForegroundState.IN_BACKGROUND;
    ...
    private void notifyListeners(AppForegroundState newState) {
        if (BuildConfig.DEBUG)
            L.ii("Notifying subscribers that app just entered state: " + newState);
    ...
    private void validateThenNotifyListeners() {
        ...
        if (mHandler.hasMessages(MESSAGE_NOTIFY_LISTENERS)) {
            if (BuildConfig.DEBUG)
                L.ii("Validation Failed: Throwing out app foreground state change notification");
        ...
        public void handleMessage(Message inputMessage) {
            switch (inputMessage.what) {
                ...
                case MESSAGE_NOTIFY_LISTENERS:
                    /* 通知所有观察者状态发生了改变 */
                    if (BuildConfig.DEBUG)
                        L.ii("App just changed foreground state to: " + mAppForegroundState);
